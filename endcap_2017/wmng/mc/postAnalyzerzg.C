				 ...
				 if(dPhiJetMET_veto(jetveto))
				   {
					 ...
					 if(lepMET_MT < 160 && (phoEt->at(phoCand[0]))/leptoMET_to_use < 1.4)
					   {
				  ...
				  if(looseEles.size() == 0)
				    {
				      ...
				      Float_t dPhi_lepMET = DeltaPhi(muPhi->at(mulist[0]),pfMETPhi);
				      Float_t lepMET_MT = sqrt(2*muPt->at(mulist[0])*MET_to_use*(1-TMath::Cos(dPhi_lepMET)));
				      if(lepMET_MT < 160 && phoEt->at(phoCandUp[0])/leptoMET_to_use < 1.4)
					{
				  ...
				  if(looseEles.size() == 0)
				    {
				      ...
				      Float_t dPhi_lepMET = DeltaPhi(muPhi->at(mulist[0]),pfMETPhi);
				      Float_t lepMET_MT = sqrt(2*muPt->at(mulist[0])*MET_to_use*(1-TMath::Cos(dPhi_lepMET)));
				      if(lepMET_MT < 160 && phoEt->at(phoCandDown[0])/leptoMET_to_use < 1.4)
					{
