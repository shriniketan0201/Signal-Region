...
void postAnalyzer::BookHistos(const char* file2)
{
  ...
  */
  ...
  for(int i=0; i<25; i++)
  {
    ...
    h_photon_eta[i] = new TH1F(("Photon_eta"+histname).c_str(), "Photon_eta",20,-5,5);h_photon_eta[i]->Sumw2();
    h_photon_SCEta[i] = new TH1F(("Photon_SCeta"+histname).c_str(), "Photon_SCeta",20,-5,5);h_photon_SCEta[i]->Sumw2();
    ...
    h_leadingJetEta[i] = new TH1F(("h_leadingJetEta"+histname).c_str(),"h_leadingJetEta",40,-5,5);h_leadingJetEta[i]->Sumw2();
